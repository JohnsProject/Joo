...
public class JooVirtualMachine {
	...
	private char[] line = new char[32];
	private char[] componentIndices = new char[TYPES_END - TYPES_START];
	private short[] components = new short[COMPONENTS_END - COMPONENTS_START];
	private char[] parameters = new char[PARAMETERS_END - PARAMETERS_START];
	private short[] arrays = new short[256];
	private boolean[] ifs = new boolean[6];
	
	// returns if successfully went to line
	private boolean goToLine(int index) {
		return false;
	}
